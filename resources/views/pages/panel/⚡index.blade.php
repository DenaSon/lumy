...
new #[Layout('layouts.panel')] class extends Component
{
    ...
    {
        return app(DashboardAnalytics::class)->defaultAccount();
    }

    #[Computed]
    public function dashboardCore(): array
    {
        return app(DashboardAnalytics::class)->buildCore($this->account);
    }

    #[Computed]
    public function dashboardAudience(): array
    {
        return app(DashboardAnalytics::class)->audienceProjection($this->account);
    }
...
<div class="space-y-6">
    <header class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if($this->account)
                    <span class="badge badge-ghost badge-sm">@{{ $this->account->username }}</span>
                @endif
            </div>
            ...
            <div @class(['space-y-6', 'hidden' => $this->account === null]) aria-busy="true">
                <div class="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm">
                    <div class="flex items-center justify-between gap-3 text-xs text-base-content/45">
                        <span>در حال آماده‌سازی Dashboard...</span>
                        <span class="loading loading-dots loading-sm"></span>
                    </div>
                    <div class="mt-4 grid grid-cols-2 gap-3 xl:grid-cols-6">
                        @for($i = 0; $i < 6; $i++)
                            <div class="animate-pulse rounded-xl bg-base-200 p-4">
                                <div class="h-3 w-20 rounded bg-base-300"></div>
                                <div class="mt-3 h-7 w-24 rounded bg-base-300"></div>
                            </div>
                        @endfor
                    </div>
                </div>
                <div class="grid gap-6 xl:grid-cols-12">
                    <div class="h-72 animate-pulse rounded-3xl bg-base-200/70 xl:col-span-7"></div>
                    <div class="h-72 animate-pulse rounded-3xl bg-base-200/70 xl:col-span-5"></div>
                </div>
                <div class="h-80 animate-pulse rounded-3xl bg-base-200/70"></div>
            </div>
        @endplaceholder

        <div class="space-y-6">
            @if($this->account !== null)
                @php($dashboard = $this->dashboardCore)
    ...
    @island(name: 'dashboard-audience', lazy: true)
        @placeholder
            <div @class(['rounded-3xl border border-base-300 bg-base-100 p-5 shadow-sm', 'hidden' => $this->account === null]) aria-busy="true">
                <div class="flex items-center justify-between gap-3 text-xs text-base-content/45">
                    <span>در حال بارگذاری Audience...</span>
                    <span class="loading loading-dots loading-sm"></span>
                </div>
                <div class="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
                    @for($i = 0; $i < 4; $i++)
                        <div class="animate-pulse rounded-2xl bg-base-200/70 p-4">
                            <div class="h-3 w-20 rounded bg-base-300"></div>
                            <div class="mt-3 h-7 w-24 rounded bg-base-300"></div>
                        </div>
                    @endfor
        ...
        <div>
            @if($this->account !== null)
                @php($audience = $this->dashboardAudience)
